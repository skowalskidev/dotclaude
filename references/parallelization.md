...
Record `agent_setup`, the actual `orchestrator_model`, worker `model`, and any `model_route` in the
living plan. Copy them into dispatch specs and handoffs, including hyperspeed parts, fresh reviewers
and retries.
...
| `full-openai` | Actual OpenAI session model | The OpenAI mid tier resolved live at dispatch by default; the Claude Fable tier for `model_route: design` | `codex -p --model <model>` / `claude -p --model fable` |
| `full-astra` | An OpenAI top-tier model | The OpenAI mid tier resolved live at dispatch for general work; the Claude Fable tier for `model_route: design` | `codex -p --model <model>` / `claude -p --model fable` |
...
- Default implementation tier: `agent_setup.resolve_tier(provider, 'mid')` — the **`sonnet`** alias for Claude, and for OpenAI whichever model currently ranks second in the live Codex model cache. Use **haiku** ONLY for genuinely mechanical Claude edits: one unambiguous rule, no taste required (a rename, a find/replace with a single correct answer).
