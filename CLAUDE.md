...
- **Always-on behavioral rules** → `~/.claude/rules/*.md` (auto-loaded every session, same priority as this file). One concern per file:
  ...
  - `process.md` — how I work: inherit the current chat's provider for every delegated role, except the Fable tier for GPT-orchestrated design work, and pick each worker's tier by the job, front-load what you need from me at session start, plan + sign-off, verify before retrying, track every task to completion (a mid-run message is queued, never an interrupt; the gauntlet plan is the checklist), commit-when-done, self-verify frontend UI changes in the browser and hand me the new UI's inputs to type myself, worktree discipline, claim ports in the shared registry, clean up processes/scratch/workspaces, project-doc rule, test/QA machine-local secrets.
  - `engineering-standards.md` — latest-stable versions, mainstream tool choice, single-source-of-truth (DRY/SRP), harden-without-breaking, legacy policy, lock-in-one-variant (no shipped A/B toggle), soft-archive-not-delete.
