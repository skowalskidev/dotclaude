...
SCENARIOS: list[dict] = [
    ...
    {"phrase": "screenshot the user flow", "expect": "ship-screenshot-changes"},
    {"phrase": "screenshot the user journey", "expect": "ship-screenshot-changes"},
    ...
    # work-split-session-in-parallel-branch-offshoot's description names /sk:work-hyperspeed (it reuses its plumbing), so the
    # word "hyperspeed" appears there — declare that overlap rather than strip the honest reference.
    {"phrase": "offshoot this", "expect": "work-split-session-in-parallel-branch-offshoot"},
    {"phrase": "work this as a branch offshoot", "expect": "work-split-session-in-parallel-branch-offshoot"},
    {"phrase": "spin this off, i might discard it", "expect": "work-split-session-in-parallel-branch-offshoot"},
    {"phrase": "explore this on a side branch i'll eyeball later", "expect": "work-split-session-in-parallel-branch-offshoot"},
    {"phrase": "don't merge it back yet", "expect": "work-split-session-in-parallel-branch-offshoot"},
