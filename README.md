...
| `bin/mockup-shell.html` + `bin/mockup-build.py` | The one spec-driven shell every `/sk:ship-mockup-before-after` mockup is built through (viewport-first stage, one collapsible rail, presentation mode, hint bar); the builder inlines a `spec.json` and extracts it back losslessly |
