...
| **The retro log** | `~/.claude/logs/retro-triggers.jsonl`, one row per session from the SessionEnd hook. Counts of guard denials, crown-jewel blocks and intake-gate stops |
| **The reconcile log** | `~/.claude/logs/intent-reconcile.jsonl`, one counts-only row per reconciled run. Read it ACROSS runs: one shortfall is noise, the same one repeatedly is a config defect. `stop_forced` is the sharpest field, because it measures the config against itself — if the Stop hook has to force the reconciliation most of the time, the written instruction is not working and the instruction is what to fix, not the enforcement |
