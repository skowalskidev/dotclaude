...
| `ship-` | Getting a change out, and reporting on it | 10 |
...
| Gauntlet loop, retain every request through tangents, independent judge or live progress dashboard | `/sk:work-gauntlet-loop` | `/sk:work-handoff-prepare-and-pickup` when a session runs low | Read `references/workflow-loops.md`. |
| Hand a running gauntlet/session to another agent (quota wall), or pick one up from a pasted dashboard link | `/sk:work-handoff-prepare-and-pickup` | — | Same-machine, chainable A→B→C. PREPARE stamps a resume manifest and returns the copy-paste dashboard link; PICK UP resolves a pasted link/dashboard.html/plan.md back to its plan and resumes the recorded engine with no second options interview. The mechanic is SSOT in `references/workflow-loops.md` § Handoff and resume; `/sk:work-gauntlet-loop` auto-detects a handoff on `--resume`. |
...
| Show production account data in a local preview before deployment, visually inspect it and leave the URL available | `/sk:ship-preview-eyeball-with-prod-data` | — | Read `references/testing-strategy.md`. |
| Verifying a data/money surface is correct on REAL prod data before shipping (a dashboard, total, count, funnel, reconciliation) | `/sk:ship-verify-with-prod-data` | — | Reads prod read-only, enumerates EVERY account the surface serves (from its own enumeration source, ranked by recent activity), recomputes what each figure would show under the new code and confirms they reconcile to one source, labels each discrepancy a code bug vs a deploy/data-skew, and supplies a user-run backfill proven first on a seedable dev account. Runs inside `/sk:ship-full-detailed-workflow`'s verification and before `/sk:ship-screenshot-changes` (re-sweep the seed account first, or the capture shows the skew fallback). Where `/sk:test-automated-full-matrix` proves the code on fixtures, this proves the numbers on real per-account data. |
