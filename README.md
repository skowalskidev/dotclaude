...
| `hooks/orphan-worker-sweep.sh` | SessionStart hook — reports (never kills) orphaned framework dev-tool AND agent-spawned worker processes machine-wide (`chrome-devtools-mcp`, `wrangler`/`workerd`, headless `codex exec`), all with a PID 1 parent. Detection delegates to `bin/kill-orphan-workers.sh --list` |
| `bin/kill-orphan-workers.sh` | Clears the orphans `hooks/orphan-worker-sweep.sh` reports — a BURNING framework worker (next/jest/vite, 20%+ CPU + 5min old, cwd in a git checkout) OR an idle-or-burning agent-spawned orphan (`chrome-devtools-mcp`, `wrangler dev`/`workerd`, headless `codex exec`), all gated on PID 1 so a live session's server is never touched. Kills the whole process family (not just the parent, which only reparents its children), re-scans for survivors, escalates to `-9`, verifies. `--dry-run` / `--list` / no-arg |
...
| `bin/dotclaude-log.py` | The one shared writer for config metrics — minimizes, boundary-scopes, appends to a local outbox first, then batch-flushes to the `dotclaude` Firestore project. Never blocks or fails a session; no-ops to the outbox when no project is configured |
...
| `references/dotclaude-setup.md` | Generic, placeholder-only setup for your own `dotclaude` Firebase project (create project, least-privilege key, venv, locked rules). Public-template-safe |
...
- Connector credentials (work), set up per project by **`/sk:setup-connectors`** (see `rules/connectors.md` + `references/connectors-setup.md`). All live OUTSIDE this repo. Each connector's manifest carries the exact recreate steps in its `auth.steps`; those are the source of truth, not this list:
  ...
  - An org key directory for work projects that share a credential: one shared `.env` (keys that reach several projects, e.g. a platform management token) plus one `<project>.env` per project. Each repo and worktree `.env` is a symlink to its project file. The directory and the recreate steps are in each work connector manifest's `secret.path` and `auth.steps`.
