...
| Same split, but you paste each part into your own separate sessions by hand | `/sk:work-hyperspeed` | `/sk:claude-config-self-optimize-analysis-after-run` after every round | Superspeed's automated `claude -p` dispatch replaced by a human relay: the orchestrator commits a clean START, writes one plan file of self-contained paste-and-forget parts, you run each in its own Conductor workspace on a branch off START, and it assembles the pushed branches and cleans them up. Reach for it when the automation is slower or less reliable than hand-running full sessions. Same self-improvement loop and slice-cutting craft as superspeed (`references/parallelization.md`); `/sk:work-warpspeed` is its VM/VPS evolution, not built. |
